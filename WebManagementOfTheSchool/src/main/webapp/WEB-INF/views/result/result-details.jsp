...
<div class="container">
	...
	<c:if test="${numberOfExams > 0}">
		<h1 class="text-center mb-5 mt-3">Chi tiết kết quả thi của lớp ${className}</h1>

		<div class="accordion" id="accordionExample">
			<c:forEach var="Time" items="${requestScope.resultTimeList}" varStatus="j"
				begin="0" step="1">
				<div class="card">
					<div class="card-header" id="heading-${Time.startDay}">
						<h2 class="mb-0">
							<button class="btn btn-link btn-block text-left" type="button"
								...
								Thời gian thi: Từ <strong>${Time.startDay}</strong> Đến <strong>${Time.endDate}</strong></button>
						</h2>
					</div>

					<div id="collapse-${Time.startDay}" class="collapse show"
						aria-labelledby="heading-${Time.startDay}" data-parent="#accordionExample">
						<div class="card-body">
								...
								<table class="table text-center">
									...
									<tbody>
										<c:forEach var="Data" items="${requestScope.resultDetails}">
											<c:if test="${Data[Time.startDay].equalsIgnoreCase(Time.startDay)}">
												<tr>
